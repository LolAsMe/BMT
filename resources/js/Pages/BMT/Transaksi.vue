            ...
            <div class="grid grid-cols-6 gap-5 place-items-center mb-5">
                ...
                <div class="block m-5 bg-white rounded-3xl drop-shadow-xl p-6 h-4/5">
                    <svg fill="#000000" width="35px" height="35px" viewBox="0 0 24 24" id="debit-purchase-left"
                        ...
                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                            <path id="secondary" d="M3,9H21a0,0,0,0,1,0,0v9a1,1,0,0,1-1,1H4a1,1,0,0,1-1-1V9A0,0,0,0,1,3,9Z"
                ...
                ">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="(transaksi, index) in transaksis" :key="transaksi.id">
                            ...
                            <td>
                                <div class="flex justify-start hover:bg-slate-300">
                                    <div class="ml-0 hover:text-blue-200 hover:cursor-pointer "
                                        @click="detail(transaksi.log_type, (transaksi.log.simpanan ? transaksi.log.simpanan.id :
                                            transaksi.log.pembiayaan ? transaksi.log.pembiayaan.id : transaksi.log.kas_id), transaksi.tanggal_transaksi,transaksi.log_id)">
                                        <div class="text-sm font-medium text-gray-900 ">
                                            {{ transaksi.log_type }}
                                        </div>
                                        <div class="text-sm text-black">
...
export default defineComponent({
    ...
    methods: {
        ...
        }, detail(type, id, tanggal = 0,log_id=0) {
            console.log(type);
            console.log(id);
            if (type == 'DetailSimpanan') {
                this.$window.target_type=type
                this.$window.target_id=id
                this.$inertia.get(this.route('simpanan.show', id), { 'filter': 'tanggal', 'filterValue': tanggal.substring(0, 10), 'id':log_id })
            } else if (type == 'DetailPembiayaan') {
                this.$window.target_type=type
                this.$window.target_id=id
                this.$inertia.get(this.route('pembiayaan.show', id),{'id':log_id})
            } else if (type == 'DetailKas') {
                this.$window.target_type=type
                this.$window.target_id=id
                this.$inertia.get(this.route('kas.index', id), { 'filter': 'tanggal', 'filterValue': tanggal.substring(0, 10), 'id':log_id })
            }
        }
    },
    data() {
